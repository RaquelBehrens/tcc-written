...
Por fim, na seção de conclusão, serão discutidos os resultados obtidos ao longo do desenvolvimento e validação da ferramenta, com uma análise crítica sobre os pontos positivos e limitações do sistema. Será abordada a eficácia da integração entre o Moodle e o Beecrowd por meio da LTI, bem como o impacto do sistema especialista no processo de aprendizagem dos alunos. Além disso, serão sugeridas possíveis melhorias para a plataforma e discutidas futuras direções para o aprimoramento da ferramenta, incluindo novas funcionalidades ou ajustes nas questões tratadas pelo sistema. A seção também refletirá sobre a contribuição do trabalho para o ensino de programação, destacando como a ferramenta pode auxiliar tanto alunos quanto docentes na resolução de dúvidas relacionadas ao Beecrowd.

\section{ESTRUTURA DO TRABALHO}

A estrutura deste trabalho é organizada da seguinte maneira: a seção de Fundamentação Teórica apresenta os conceitos essenciais para compreender o desenvolvimento do projeto. Em Trabalhos Relacionados, são comparadas abordagens de integração de juízes online com o Moodle, além de artigos que tratam do desenvolvimento de sistemas especialistas web. O capítulo de Desenvolvimento apresenta a arquitetura da integração LTI, as tecnologias utilizadas e a implementação do sistema especialista. Na seção de Validação, são avaliadas as vantagens e limitações do sistema, com foco na aplicação pelos alunos. A Conclusão discute os resultados, limitações, melhorias e a contribuição do trabalho para o ensino de programação.
